...
class Replicate(commands.Cog):
  ...
  async def command_slash(self, interaction: nextcord.Interaction, model, prompt):
  ...
  async def command_text(self, ctx, model, *, prompt):
    ...
    if image:
      ...
      with open(video_path, 'rb') as f:
        await ctx.send(
            ...
            f"**Reference image:** [{file_name}](<{img_prompt_url}>)\n"
            f"**Model:** `{selected_model}`\n"
            f"Frames: `{video_length}`\n" if video_length else ""
            "\n"
            f"---------------------------\n"
    ...
    # # Create an embed to display the image in Discord
    # embed = nextcord.Embed(title="Generated Image",
    ...
    await self.command(ctx, model)

  async def command(self, interaction, model):
    ...
    if isinstance(interaction, commands.Context):
      ...
      edit_message = interaction.edit
      edit_after_defer = None
      delete_message = interaction.delete
